 ...
 *
 * WAAROM DIT MAG. Logistische regressie is lineair, dus een voorspelling is
 * `softmax(gewichten . pixels + constante)` en niets meer. De som hieronder is
 * nagerekend tegen `predict_proba` van sklearn met exact deze gewichten, over
 * de 120 beeldjes van het exportbestand in negen verschuivingen: 0 afwijkende
 * keuzes van 1080, grootste verschil in kans 0. Het bord rekent dus met HET
 * model van de les, niet met een benadering ervan.
 *
 * DAT HET HET MODEL VAN DE LES IS, is ook aan de gewichten zelf te zien. De les
 ...
 * |gewicht| 9,6e-03 en staat 30% van de gewichten onder 1e-04. Het
 * exportbestand rondt daarom af op 4 BEDUIDENDE cijfers en nooit op 4
 * decimalen - dat laatste zou een derde van het model op nul zetten.
 *
...
/**
 * Hoe zeker het model is van elk cijfer, samen precies 1.
 *
 * De nulpixels worden overgeslagen. Dat is geen benadering maar dezelfde som:
 * een pixel van 0 draagt 0 bij. Het scheelt wel een factor 5, en dat is wat
 * de teller over alle beeldjes bij elke klik betaalbaar houdt.
 */
export function zekerheden(model: PixelModel, beeld: Uint8Array): Float64Array {
  ...
  let top = -Infinity
  for (let c = 0; c < n; c++) if (uit[c] > top) top = uit[c]
  let totaal = 0
  for (let c = 0; c < n; c++) {
    uit[c] = Math.exp(uit[c] - top)
    totaal += uit[c]
  }
  for (let c = 0; c < n; c++) uit[c] /= totaal
  return uit
}

/** Het cijfer waar het model voor kiest: de hoogste balk. */
export function keuze(zeker: Float64Array): number {
  let beste = 0
  for (let c = 1; c < zeker.length; c++) if (zeker[c] > zeker[beste]) beste = c
  return beste
}
...
export function keuzes(model: PixelModel, dy: number, dx: number): Int8Array {
  ...
  for (let i = 0; i < model.beelden.length; i++) {
    uit[i] = keuze(zekerheden(model, verschuif(model.beelden[i], dy, dx)))
  }
  return uit
}
